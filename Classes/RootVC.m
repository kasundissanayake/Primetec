...
- (void)changeQuantitySummary:(NSNotification *)aNotif
{
    NSDictionary *aDict = [aNotif userInfo];
...
- (void)changeDExpeseForm:(NSNotification *)aNotification
{
    NSDictionary *aDict = [aNotification userInfo];
    ExpenceViewController *expence=[[ExpenceViewController alloc]init];
    if(aDict)
        expence.exNUmber = [aDict objectForKey:@"eXReportNo"];
    else
        expence.exNUmber = nil;
    expence.title=[NSString stringWithFormat:@"Expense Report"];
    [self.detailedNavigationController setViewControllers:[NSArray arrayWithObject:expence]];
}


- (void)changeSummaryForm:(NSNotification *)aNotif
{
    NSDictionary *aDict = [aNotif userInfo];
